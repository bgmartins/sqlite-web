...
FROM TEMP_ENTIDADES LEFT JOIN TEMP_DATAS_ENTIDADES ON TEMP_ENTIDADES.ID_ENTIDADE=TEMP_DATAS_ENTIDADES.ID_ENTIDADE WHERE instr(NOME_ENTIDADE, 'SIND') <= 0 AND instr(NOME_ENTIDADE, 'TRABALH') <= 0 AND instr(NOME_ENTIDADE, 'PROFISSI') <= 0 AND instr(NOME_ENTIDADE, 'CGTPIN') <= 0;
...
SELECT TEMP_ENTIDADES.ID_ENTIDADE, 
       ...
       MAX_DATA,
       CASE lower(trim(ESTADO_ENTIDADE)) WHEN 'ativa' THEN 1 ELSE 0 END
FROM TEMP_ENTIDADES LEFT JOIN TEMP_DATAS_ENTIDADES ON TEMP_ENTIDADES.ID_ENTIDADE=TEMP_DATAS_ENTIDADES.ID_ENTIDADE WHERE instr(NOME_ENTIDADE, 'SIND') > 0 OR instr(NOME_ENTIDADE, 'TRABALH') > 0 OR instr(NOME_ENTIDADE, 'PROFISSI') > 0 OR instr(NOME_ENTIDADE, 'CGTPIN') > 0;
...
SELECT DISTINCT TEMP_ENTIDADES.ID_ENTIDADE,
       ...
       0 AS Eleicoes,
       1 AS Confianca
FROM TEMP_ENTIDADES NATURAL JOIN TEMP_ALTERACOES_ESTATUTOS WHERE instr(NOME_ENTIDADE, 'SIND') > 0 OR instr(NOME_ENTIDADE, 'TRABALH') > 0 OR instr(NOME_ENTIDADE, 'PROFISSI') > 0 OR instr(NOME_ENTIDADE, 'CGTPIN') > 0
UNION
SELECT DISTINCT TEMP_ENTIDADES.ID_ENTIDADE,
       strftime('%Y',date(replace(DATABTE,'.','-'))) AS Ano,
       NUMBTE AS Numero,
       ...
       0 AS Eleicoes,
       1 AS Confianca
FROM TEMP_ENTIDADES NATURAL JOIN TEMP_ELEICAO_CORPOS_GERENTES WHERE instr(NOME_ENTIDADE, 'SIND') > 0 OR instr(NOME_ENTIDADE, 'TRABALH') > 0 OR instr(NOME_ENTIDADE, 'PROFISSI') > 0 OR instr(NOME_ENTIDADE, 'CGTPIN') > 0;
...
SELECT DISTINCT TEMP_ENTIDADES.ID_ENTIDADE,
       ...
       0 AS Eleicoes,
       1 AS Confianca
FROM TEMP_ENTIDADES NATURAL JOIN TEMP_ALTERACOES_ESTATUTOS WHERE instr(NOME_ENTIDADE, 'SIND') <= 0 AND instr(NOME_ENTIDADE, 'TRABALH') <= 0 AND instr(NOME_ENTIDADE, 'PROFISSI') <= 0 AND instr(NOME_ENTIDADE, 'CGTPIN') <= 0
UNION
SELECT DISTINCT TEMP_ENTIDADES.ID_ENTIDADE,
       strftime('%Y',date(replace(DATABTE,'.','-'))) AS Ano,
       NUMBTE AS Numero,
       ...
       0 AS Eleicoes,
       1 AS Confianca
FROM TEMP_ENTIDADES NATURAL JOIN TEMP_ELEICAO_CORPOS_GERENTES WHERE instr(NOME_ENTIDADE, 'SIND') <= 0 AND instr(NOME_ENTIDADE, 'TRABALH') <= 0 AND instr(NOME_ENTIDADE, 'PROFISSI') <= 0 AND instr(NOME_ENTIDADE, 'CGTPIN') <= 0;
...
CREATE TABLE CAE_SECCOES_TEMP( SECCAO CHAR(1) PRIMARY KEY , TITLE VARCHAR(100) );
CREATE TABLE CAE_SECCOES_KEYWORDS_TEMP( SECCAO CHAR(1) , KEYWORD VARCHAR(100) , FOREIGN KEY(SECCAO) REFERENCES CAE_SECCOES_TEMP );

INSERT INTO CAE_SECCOES_TEMP VALUES ( 'A' , 'AGRICULTURA, PRODUÇÃO ANIMAL, CAÇA, FLORESTA E PESCA');
INSERT INTO CAE_SECCOES_TEMP VALUES ( 'B' , 'INDÚSTRIAS EXTRACTIVAS');
INSERT INTO CAE_SECCOES_TEMP VALUES ( 'C' , 'INDÚSTRIAS TRANSFORMADORAS');
INSERT INTO CAE_SECCOES_TEMP VALUES ( 'D' , 'ELECTRICIDADE, GÁS, VAPOR, ÁGUA QUENTE E FRIA E AR FRIO');
INSERT INTO CAE_SECCOES_TEMP VALUES ( 'E' , 'CAPTAÇÃO, TRATAMENTO E DISTRIBUIÇÃO DE ÁGUA; SANEAMENTO GESTÃO DE RESÍDUOS E DESPOLUIÇÃO');
INSERT INTO CAE_SECCOES_TEMP VALUES ( 'F' , 'CONSTRUÇÃO');
INSERT INTO CAE_SECCOES_TEMP VALUES ( 'G' , 'COMÉRCIO POR GROSSO E A RETALHO; REPARAÇÃO DE VEÍCULOS AUTOMÓVEIS E MOTOCICLOS');
INSERT INTO CAE_SECCOES_TEMP VALUES ( 'H' , 'TRANSPORTES E ARMAZENAGEM');
INSERT INTO CAE_SECCOES_TEMP VALUES ( 'I' , 'ALOJAMENTO, RESTAURAÇÃO E SIMILARES');
INSERT INTO CAE_SECCOES_TEMP VALUES ( 'J' , 'ACTIVIDADES DE INFORMAÇÃO E DE COMUNICAÇÃO');
INSERT INTO CAE_SECCOES_TEMP VALUES ( 'K' , 'ACTIVIDADES FINANCEIRAS E DE SEGUROS');
INSERT INTO CAE_SECCOES_TEMP VALUES ( 'L' , 'ACTIVIDADES IMOBILIÁRIAS');
INSERT INTO CAE_SECCOES_TEMP VALUES ( 'M' , 'ACTIVIDADES DE CONSULTORIA, CIENTÍFICAS, TÉCNICAS E SIMILARES');
INSERT INTO CAE_SECCOES_TEMP VALUES ( 'N' , 'ACTIVIDADES ADMINISTRATIVAS E DOS SERVIÇOS DE APOIO');
INSERT INTO CAE_SECCOES_TEMP VALUES ( 'O' , 'ADMINISTRAÇÃO PÚBLICA E DEFESA; SEGURANÇA SOCIAL OBRIGATÓRIA');
INSERT INTO CAE_SECCOES_TEMP VALUES ( 'P' , 'EDUCAÇÃO');
INSERT INTO CAE_SECCOES_TEMP VALUES ( 'Q' , 'ACTIVIDADES DE SAÚDE HUMANA E APOIO SOCIAL');
INSERT INTO CAE_SECCOES_TEMP VALUES ( 'R' , 'ACTIVIDADES ARTÍSTICAS, DE ESPECTÁCULOS, DESPORTIVAS E RECREATIVAS');
INSERT INTO CAE_SECCOES_TEMP VALUES ( 'S' , 'OUTRAS ACTIVIDADES DE SERVIÇOS');
INSERT INTO CAE_SECCOES_TEMP VALUES ( 'T' , 'ACTIVIDADES DAS FAMÍLIAS EMPREGADORAS DE PESSOAL DOMÉSTICO E ACTIVIDADES DE PRODUÇÃO DAS FAMÍLIAS PARA USO PRÓPRIO');
INSERT INTO CAE_SECCOES_TEMP VALUES ( 'U' , 'ACTIVIDADES DOS ORGANISMOS INTERNACIONAIS E OUTRAS INSTITUIÇÕES EXTRA-TERRITORIAIS');
...
INSERT INTO Sectores_Profissionais SELECT TITLE AS Sector, NULL FROM CAE_SECCOES_TEMP;
...
INSERT INTO TEMP_MCP SELECT DESIG, CP4 || '-' || CP3 || '-' AS CP FROM TEMP_MUNICIPIOS NATURAL JOIN TEMP_CP UNION SELECT DESIG, CP4 || '-' AS CP FROM TEMP_MUNICIPIOS NATURAL JOIN TEMP_CP;
UPDATE Org_Sindical SET Codigo_Postal=Codigo_Postal || '-' WHERE Codigo_Postal IS NOT NULL;
UPDATE Org_Patronal SET Codigo_Postal=Codigo_Postal || '-' WHERE Codigo_Postal IS NOT NULL;
...
UPDATE Org_Sindical SET Concelho_Sede=(SELECT DISTINCT DESIG FROM TEMP_MCP WHERE Codigo_Postal = CP);
UPDATE Org_Patronal SET Concelho_Sede=(SELECT DISTINCT DESIG FROM TEMP_MCP WHERE Codigo_Postal = CP);
DROP INDEX TMPIDX1;
DROP INDEX TMPIDX2;
DROP INDEX TMPIDX3;
DROP TABLE TEMP_MCP;
